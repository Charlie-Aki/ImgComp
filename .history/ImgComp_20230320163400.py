    ...
    def img_comp(self, thread_flag, fname1, fname2):
        ...
        for j in range(len(self.fname1)):
            if self.thread_flag.is_set() == True:
                ...
                #画像バイナリデータの読み込み
                self.drw1, self.drw2, num_frames = self.read_binary_image(self.fname1[j], self.fname2[j])
                if self.file_exts[0].endswith(".tif") or self.file_exts[0].endswith(".tiff"):
                    ...
                    self.drw1 = pdf2image.convert_from_path(self.fname1[j], dpi=300)
                    self.drw2 = pdf2image.convert_from_path(self.fname2[j], dpi=300)
