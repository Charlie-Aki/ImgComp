...
class MainFrame(ttk.Frame):
    ...
    def run_program(self):
        try:
            ...
            ######################
            self.file_exts = aida_func.detect_exts(self.fname1, self.fname2)
            #エントリーボックスが空
            ...
            thread_main = threading.Thread(target=aida_func.img_comp(thread_flag=self.thread_flag, fname1=self.fname1, fname2=self.fname2))
...
class AidaCore:
    def __init__(self, ctrl_frame, ctrl_frame2):
        self.ctrl_frame=ctrl_frame # progress_frame
        self.ctrl_frame2=ctrl_frame2 # main_frame
    ...
    def img_comp(self, thread_flag, fname1, fname2):
        """
        画像の画素差分出力
        """
        ...
        self.fname1=fname1
        self.fname2=fname2
        self.file_exts = self.detect_exts(self.fname1, self.fname2)
        ...
        for j in range(len(self.fname1)):
            if self.thread_flag.is_set() == True:
                ...
                #画像バイナリデータの読み込み
                self.drw1, self.drw2, num_frames, dpi_value = self.read_binary_image(self.fname1[j], self.fname2[j])

                #pdf2image上限エラー
                if self.file_exts[0].endswith(".pdf") and num_frames > 100:
                    self.ctrl_frame2.regular_error("PDFファイルは100ページまでしか読み込めません。")
                    return
                ...
                if self.thread_flag.is_set() == True:
                    ...
                    for page in range(0, num_frames):
                        scroll_txt = str(j+1)+"/"+str(len(self.fname1))+"ファイルの" +\
                            str(page+1)+"/"+str(num_frames)+"ページ目を処理中..."
                        self.ctrl_frame.text_message(scroll_txt)

                        #画像のグレースケール化＋出力用変数の準備
                        self.drw1_gray, self.drw2_gray=self.gray_scale(self.drw1[page], self.drw2[page])
                        self.drw_out = self.drw2_gray.convert("RGB")
                        if self.drw1_gray.size != self.drw2_gray.size:
                            self.ctrl_frame2.regular_error("同じサイズの画像を選択してください")
                            return

                        #差分を色塗りするためのマスクの作成
                        mask_red = ImageChops.subtract(self.drw1_gray, self.drw2_gray)
                        mask_blue = ImageChops.subtract(self.drw2_gray, self.drw1_gray)

                        #マスク使用して差分箇所に色塗り
                        self.drw_out.paste(Image.new("RGB", self.drw_out.size, "red"), mask=mask_red)
                        self.drw_out.paste(Image.new("RGB", self.drw_out.size, "blue"), mask=mask_blue)
                        ...
                        self.drw_text.text((50, 50), self.txt, fill="red", font=fnt)
                        self.txt = os.path.splitext(os.path.basename(self.fname1[j]))[0]+' => '
                        self.drw_text.text((50, 50), self.txt, fill="black", font=fnt)
                        self.txt = os.path.splitext(os.path.basename(self.fname1[j]))[0]
                        self.drw_text.text((50, 50), self.txt, fill="blue", font=fnt)
                    ...
                    if self.thread_flag.is_set() == True:
                        ...
                        if self.thread_flag.is_set() == True:
                            self.saveTiffStack(self.tiff_name, self.drw_out_4dim_pages, dpi_value)
    ...
    def gray_scale(self, drw1, drw2):
        drw1_gray = drw1.convert("L")
        drw2_gray = drw2.convert("L")
        return drw1_gray, drw2_gray
