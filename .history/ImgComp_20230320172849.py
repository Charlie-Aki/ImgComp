...
class AidaCore:
            ...
            if self.thread_flag.is_set() == True:
                ...
                if self.thread_flag.is_set() == True:
                    ...
                    for page in range(0, num_frames):
                        ...
                        if self.drw1[page].size != self.drw2[page].size:
                            self.ctrl_frame2.regular_error("同じサイズの画像を選択してください")
                            return

                        #新旧青/赤色付け
                        self.drw_out=self.create_masked_image(self.drw1[page], self.drw2[page])
                        #左上文字入れ
                        self.drw_out = text_insert(self, drw_out, fname1, fname2)
                        #1ファイルにまとめ
    ...
    def text_insert(self, drw_out, fname1, fname2):
        ...
        txt = os.path.splitext(os.path.basename(self.fname1[j]))[0]+' => '\
