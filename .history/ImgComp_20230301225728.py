...
class MainFrame(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        # old_entry.dnd_bind('<<Drop>>', functools.partial(self.drop_files, focus_entry=self.old_entry))
        # new_entry.drop_target_register(tkdnd.DND_FILES)
        # new_entry.dnd_bind('<<Drop>>', functools.partial(self.drop_files, focus_entry=self.new_entry))
        # outdir_entry.drop_target_register(tkdnd.DND_FILES)
        # outdir_entry.dnd_bind('<<Drop>>', functools.partial(self.drop_folder, focus_entry=self.outdir_entry))
        ...
        check_tiff = ttk.Checkbutton(self, text="tiff", variable=outext1, onvalue="tiff_on", offvalue="tiff_off", command=lambda: print(self.settings["outext1"]))
