...
class How2UseWindow(tk.Toplevel):
    def __init__(self, master):
        ...
        self.im_width, self.im_height = self.imgs[0].size
        # ボタン・セパレータ・ページ進みの行の高さが46みたい
        self.geometry(f'{int(self.im_width*0.75)}x{int(self.im_height*0.75)+46}')
        self.minsize(f'{int(self.im_width*0.25)}', f'{int(self.im_height*0.25)+46}')
    ...
    def create_widgets(self):
        ...
        imgs_copy=self.image_copy(self.imgs)
    ...
    def resize_image(self, event, image_number):
        ...
        aspect = self.im_width/float(self.im_height)
        new_height = event.height
        new_width = int(new_height*aspect)
        if new_width > event.width:
            new_width = event.width
            new_height = int(new_width/aspect)
        self.imgs[image_number] = imgs_copy[image_number].resize((new_width, new_height))
        how2_imgs[image_number] = ImageTk.PhotoImage(self.imgs[image_number])
    ...
    def forward(self, image_number):
        ...
        img_label.bind('<Configure>', functools.partial(self.resize_image,image_number=image_number-1))
