...
class How2UseWindow(tk.Toplevel):
    def __init__(self, master):
        super().__init__(master)
        self.title("使い方")
        self.iconbitmap(self.icon_file_path)  # Works only on Windows OS
        ...
        self.how2use_window.geometry(
            f'{int(im_width*0.75)}x{int(im_height*0.75)+46}')
        self.how2use_window.minsize(
            f'{int(im_width*0.25)}', f'{int(im_height*0.25)+46}')

        self.img_label = ttk.Label(
            self.how2use_window, image=self.how2_img[0], anchor='center', background='white')
        self.img_label.bind('<Configure>', functools.partial(
            self.resize_image, image_number=0))
        self.button_back = ttk.Button(
            self.how2use_window, text="<<", command=self.back, state=tk.DISABLED)
        self.button_exit = ttk.Button(
            self.how2use_window, text="閉じる", command=self.how2use_window.destroy)
        self.button_forward = ttk.Button(
            self.how2use_window, text=">>", command=lambda: self.forward(2))
        self.separator = ttk.Separator(
            self.how2use_window, orient=tk.HORIZONTAL)
        self.status = ttk.Label(
            self.how2use_window, text="ページ 1 / " + str(len(self.how2_img)), anchor='e')
