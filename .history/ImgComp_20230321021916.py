    ...
    def img_comp_flow(self, fname1, fname2, output_dir):
        ...
        for j in range(len(fname1)):
            if self.thread_flag.is_set() == True:
                ...
                #画像バイナリデータの読み込み
                drw1, drw2, num_frames, dpi_value = aida_func.read_binary_image(fname1[j], fname2[j])
...
class AboutWindow(tk.Toplevel):
    def __init__(self, master):
        ...
        # self.grid_rowconfigure(0, weight=0)
        # self.grid_rowconfigure(1, weight=0)
        self.logo_img = Image.open(self.master.icon_image)
        self.logo_img = self.logo_img.resize((100, 100), Image.LANCZOS)
        self.tk_logo_img = ImageTk.PhotoImage(self.logo_img)
        self.create_widgets()
