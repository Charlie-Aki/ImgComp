...
class MainFrame(ttk.Frame):
    ...
    def run_program(self):
        try:
            ...
            thread_main = threading.Thread(target=self.img_comp(thread_flag=self.thread_flag, fname1=self.fname1, fname2=self.fname2))
    ...
    def img_comp(self, thread_flag, fname1, fname2):
        ...
        for j in range(len(self.fname1)):
            if self.thread_flag.is_set() == True:
                ...
                #画像バイナリデータの読み込み
                self.drw1, self.drw2, num_frames, dpi_value = self.read_binary_image(
                    self.fname1[j], self.fname2[j])

                #pdf2image上限エラー
                if self.file_exts[0].endswith(".pdf") and num_frames > 100:
                    self.ctrl_frame2.regular_error(
                ...
                if self.thread_flag.is_set() == True:
                    ...
                    for page in range(0, num_frames):
                        ...
                        #新旧青/赤色付け
                        self.drw_out = self.create_masked_image(
                            self.drw1[page], self.drw2[page])
                        #左上文字入れ
                        self.drw_out = self.insert_text(
                            self, self.drw_out, self.fname1[j], self.fname2[j])
                        #配列内にまとめる
                    ...
                    if self.thread_flag.is_set() == True:
                        ...
                        self.opened_file_check(self.tiff_name)
                        if self.thread_flag.is_set() == True:
                            self.saveTiffStack(
                                self.tiff_name, self.drw_out_4dim_pages, dpi_value)
                        ...
                        if self.outext2.get() == "pdf_on":
                            ...
                            # 保存先で同一名ファイルが開かれていないかチェックする
                            self.opened_file_check(self.save_path)
                            if self.thread_flag.is_set() == True:
                                self.savePDFStack(
                                    self.save_path, self.tiff_name)
