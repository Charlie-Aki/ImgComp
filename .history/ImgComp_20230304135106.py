...
class How2UseWindow(tk.Toplevel):
    def __init__(self, master):
        ...
        img_label = ttk.Label(
            self, image=how2_imgs[0], anchor='center', background='white')
        img_label.bind('<Configure>', functools.partial(
            self.resize_image, image_number=0))
        button_back = ttk.Button(
            self, text="<<", command=self.back, state=tk.DISABLED)
        button_exit = ttk.Button(
            self, text="閉じる", command=self.destroy)
        button_forward = ttk.Button(
            self, text=">>", command=lambda: self.forward(2))
        separator = ttk.Separator(
            self, orient=tk.HORIZONTAL)
        status = ttk.Label(
            self, text="ページ 1 / " + str(len(how2_imgs)), anchor='e')
