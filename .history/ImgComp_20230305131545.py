...
class How2UseWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        for i_cp in range(len(self.imgs)):
            imgs_cp.append(self.imgs[i_cp].copy())
            how2_imgs.append(ImageTk.PhotoImage(self.imgs[i_cp]))
        im_width, im_height = imgs[0].size
        aspect = im_width/float(im_height)
        # ボタン・セパレータ・ページ進みの行の高さが46みたい
        self.geometry(
            f'{int(im_width*0.75)}x{int(im_height*0.75)+46}')
        self.minsize(
            f'{int(im_width*0.25)}', f'{int(im_height*0.25)+46}')

        img_label = ttk.Label(self, image=how2_imgs[0], anchor='center', background='white')
        img_label.bind('<Configure>', functools.partial(self.resize_image, image_number=0, aspect=aspect))
