...
class Application(tkdnd.TkinterDnD.Tk):
    def __init__(self):
        super().__init__()
        aida_func=AidaCore()
        self.icon_image=aida_func.get_abs_path('./imgs/ImgComp_256x256.ico').replace('/', os.sep)
...
class Menubar(tk.Menu):
    ...
    def show_how2use_window(self):
        self.how2use_window=How2UseWindow(master=self.master)

    def show_about_window(self):
        self.about_window=AboutWindow(master=self.master)
...
class How2UseWindow(tk.Toplevel):
    def __init__(self, master):
        ...
        self.imgs_copy=self.image_copy(self.imgs)
        self.create_widgets()
    ...
    def create_widgets(self):
        ...
        for i_cp in range(len(self.imgs)):
            how2_imgs.append(ImageTk.PhotoImage(self.imgs[i_cp]))
        im_width, im_height = self.imgs[0].size

        # ボタン・セパレータ・ページ進みの行の高さが46みたい
        self.geometry(f'{int(im_width*0.75)}x{int(im_height*0.75)+46}')
        self.minsize(f'{int(im_width*0.25)}', f'{int(im_height*0.25)+46}')

        img_label = ttk.Label(self, image=how2_imgs[0], anchor='center', background='white')
        img_label.bind('<Configure>', functools.partial(self.resize_image, image_number=0))
        button_back = ttk.Button(self, text="<<", command=self.back, state=tk.DISABLED)
        button_exit = ttk.Button(self, text="閉じる", command=self.destroy)
        button_forward = ttk.Button(self, text=">>", command=lambda: self.forward(2))
        separator = ttk.Separator(self, orient=tk.HORIZONTAL)
        status = ttk.Label(self, text="ページ 1 / " + str(len(how2_imgs)), anchor='e')
    ...
    def image_copy(self, imgs):
        """
        pillowモジュールは何か変更を加えると元のデータが上書きされてしまうので基本的にコピーを作って作業する。
        """
        imgs_copy=[]
    ...
    def resize_image(self, event, image_number, aspect):
        # imgs
        # imgs_cp
        # how2_imgs
        # image_number
        ...
        aspect = im_width/float(im_height)
        new_height = event.height
        new_width = int(new_height*aspect)
        if new_width > event.width:
            new_width = event.width
            new_height = int(new_width/aspect)
        imgs[image_number] = imgs_cp[image_number].resize((new_width, new_height))
        how2_imgs[image_number] = ImageTk.PhotoImage(imgs[image_number])
        img_label.config(image=how2_imgs[image_number])
        # avoid garbage collection
        img_label.image = how2_imgs[image_number]

    def forward(self, image_number, aspect):
        # image_number
        # img_label
        # how2_imgs

        img_label.grid_forget()
        img_label = ttk.Label(self, image=how2_imgs[image_number-1], anchor='center', background='white')
        img_label.bind('<Configure>', functools.partial(self.resize_image,image_number=image_number-1, aspect=aspect))
        button_forward = ttk.Button(self, text=">>", command=lambda: self.forward(image_number+1))
        button_back = ttk.Button(self, text="<<", command=lambda: self.back(image_number-1))
        if image_number == len(how2_imgs):
            button_forward = ttk.Button(self, text=">>", state=tk.DISABLED)
        separator = ttk.Separator(self, orient=tk.HORIZONTAL)
        status = ttk.Label(self, text="ページ " + str(image_number) + " / " + str(len(how2_imgs)), anchor='e')
    ...
    def back(self, image_number, aspect):
        # image_number
        # aspect
        ...
        button_back = ttk.Button(self, text="<<", command=lambda: self.back(image_number-1))
        if image_number == 1:
            button_back = ttk.Button(self, text="<<", state=tk.DISABLED)
        separator = ttk.Separator(self, orient=tk.HORIZONTAL)
        status = ttk.Label(self, text="ページ " + str(image_number) + " / " + str(len(how2_imgs)), anchor='e')
...
class AboutWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        # self.grid_rowconfigure(0, weight=0)
        # self.grid_rowconfigure(1, weight=0)
        original_icon_img = Image.open(self.master.icon_image)
        original_icon_img = original_icon_img.resize((100, 100), Image.LANCZOS)
        tk_icon_img = ImageTk.PhotoImage(original_icon_img)

        icon_label = ttk.Label(self, image=tk_icon_img, anchor='e')
