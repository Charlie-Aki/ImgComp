...
class MainFrame(ttk.Frame):
    ...
    def run_program(self, thread_flag):
        try:
            ...
            thread_main = threading.Thread(target=self.img_comp(thread_flag=thread_flag, fname1=fname1, fname2=fname2, output_dir=output_dir))
            thread_flag.set()
            thread_main.start()
    ...
    def img_comp(self, thread_flag, fname1, fname2, output_dir):
        """
        画像の画素差分出力
        """
            ...
            if thread_flag.is_set() == True:
                ...
                if thread_flag.is_set() == True:
                    ...
                    for page in range(0, num_frames):
                        ...
                        #新旧青/赤色付け
                        self.drw_out = aida_func.create_masked_image(
                            self.drw1[page], self.drw2[page])
                        #左上文字入れ
                        self.drw_out = aida_func.insert_text(
                            self.drw_out, fname1[j], fname2[j])
                        #配列内にまとめる
                        drw_out_4dim_pages.append(self.drw_out)

                    if thread_flag.is_set() == True:
                        #出力先に保存
                        scroll_txt = "出力先にファイルを保存しています..."
                        self.ctrl_frame.text_message(scroll_txt)
                        self.tiff_name = os.path.join(output_dir, "Output_"
                                                      + os.path.splitext(os.path.basename(fname2[j]))[0] + ".tif")
                        # 保存先で同一名ファイルが開かれていないかチェックする
                        self.opened_file_check(self.tiff_name)
                        if thread_flag.is_set() == True:
                            self.saveTiffStack(
                                self.tiff_name, drw_out_4dim_pages, dpi_value)
                        #pdfにチェックがあればPDFに変換する
                        if self.outext2.get() == "pdf_on":
                            self.save_path = os.path.join(output_dir, "Output_"
