...
class Application(tkdnd.TkinterDnD.Tk):
    def __init__(self):
        ...
        self.iconbitmap(AidaCore.get_abs_path('./imgs/ImgComp_256x256.ico').replace('/', os.sep)) # Windows用アイコン設定
...
class MainFrame(ttk.Frame):
    ...
    def specify_style(self):
        ...
        self.style.configure('main_frame.theme', background='red')

    def create_widgets(self):
        self.main_frame = ttk.Frame(self.master, style='main_frame.theme')
        self.main_frame.grid(row=0, column=0, padx=10, pady=5, sticky='EW')
        self.main_frame.grid_columnconfigure(1, weight=1)
        self.main_frame.grid_columnconfigure(2, weight=1)

        # Create Labels for Main Frame
        self.old_dialogue_label = ttk.Label(self.main_frame, text="旧図ファイル選択:")
        self.new_dialogue_label = ttk.Label(self.main_frame, text="改訂図ファイル選択:")
        self.outdir_dialogue_label = ttk.Label(self.main_frame, text="出力先フォルダ選択:")
        self.outext_label = ttk.Label(self.main_frame, text="出力形式:")
        ...
        self.old_entry = ttk.Entry(self.main_frame, width=30)
        self.old_entry.grid(row=0, column=1, columnspan=2, sticky='EW')
        self.new_entry = ttk.Entry(self.main_frame, width=30)
        self.new_entry.grid(row=1, column=1, columnspan=2, sticky='EW')
        self.outdir_entry = ttk.Entry(self.main_frame, width=30)
        ...
        self.old_dialogue_button = ttk.Button(self.main_frame, text="参照", command=self.old_entry_dialogue)
        self.new_dialogue_button = ttk.Button(self.main_frame, text="参照", command=self.new_entry_dialogue)
        self.outdir_dialogue_button = ttk.Button(self.main_frame, text="参照", command=self.outdir_entry_dialogue)
        self.old_clear_button = ttk.Button(self.main_frame, text="クリア", command=lambda: self.old_entry.delete(0, tk.END))
        self.new_clear_button = ttk.Button(self.main_frame, text="クリア", command=lambda: self.new_entry.delete(0, tk.END))
        self.outdir_clear_button = ttk.Button(self.main_frame, text="クリア", command=lambda: self.outdir_entry.delete(0, tk.END))
        self.run_button = ttk.Button(self.main_frame, text="実行", command=self.start_thread_main)
        self.stop_button = ttk.Button(self.main_frame, text="中断", command=self.stop_program)
        ...
        self.check_tiff = ttk.Checkbutton(self.main_frame, text="tiff", variable=self.outext1, onvalue="tiff_on", offvalue="tiff_off")
        self.check_pdf = ttk.Checkbutton(self.main_frame, text="pdf", variable=self.outext2, onvalue="pdf_on", offvalue="pdf_off")
